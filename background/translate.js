...
function getTrans(queryArray,successFun){
    let appid = '20180425000150418';
    let key = 'egFj6Vwsu8JRYEMqFApg';
    ...
    myAjax.get(target).then((responseText)=>{
        ...
        let trans_result = data.trans_result[0];

        //将源词汇和翻译结果整合成字典对象（key-value 对象）
        /*let srcArray = trans_result["src"].split("!")
        let dstArray = trans_result["dst"].split("！")//中文 "！"；*/
        let srcArray = trans_result["src"].split(",")
        let dstArray = trans_result["dst"].split("，")//中文 "，"；
        let trans_result_map = {}
...
function sucessFun(trans_result){
    ...
    lastWedPageTranslateResult = trans_result
}

var lastWedPageTranslateResult;
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse){
    switch (message.type){
        ...
        case "vocabularies":
            ...
            break
        case "storeVocabularies":
            let voc = message.value
            localStorage[voc.src] = voc.dst
            sendResponse("200 ok")
    }
